...
typedef struct courses //define type (struct course), (course) can be ignored
{
    ...
    char title[20];
    float hours;
}icourse; //name of type defined, not name of variable here
...
int main()
{
    ...
    struct course english={.id=114,.title="english",.hours=40.0}; //using member to declare & initialize
    ...
    icourse cs3;
    update_icourse(&cs3); //argument by pointer
    display_icourse(cs3); //argument by value
